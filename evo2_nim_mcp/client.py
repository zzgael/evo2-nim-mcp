...
class NimClient:
    ...
    async def _get(self, path: str, *, retries: int = 2) -> NimResponse:
    ...
    async def _request(
        ...
        *,
        ...
        """One request with exponential backoff on 5xx and transient network errors.

        4xx are surfaced immediately with the response body as the error message —
        these are caller mistakes (wrong layer name, malformed sequence, etc.) and
        ...
        for attempt in range(retries + 1):
            ...
            if 400 <= resp.status_code < 500:
                # Caller error — surface immediately, never retry
